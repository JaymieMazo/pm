     ...
     With hflxPOSearch
          ...
          For intCol = 0 To .Cols - 1
               .ColWidth(intCol) = Choose(intCol + 1, 1200, 700, 1000, 4000, 1800, 1800, 1800, 1000, 2500, 2000, _
                         1000, 1500, 1500, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 4000, 1500, 1500)
               .ColAlignment(intCol) = Choose(intCol + 1, 4, 4, 4, 1, 7, 7, 7, 4, 1, 1, 1, 1, 1, 4, 4, 4, 4, 4, 4, 4, 4, 4, _
                                                                    4, 4, 4)
               .Col = intCol
               .CellAlignment = 4
               .CellFontBold = True
...
' Modified by Tubs 20180321
        strSQLQuery = "SELECT POSearchView.PoNo,POSearchView.PoSeq,POSearchView.ItemId," _
                    ...
                    & " POSearchView.Remarks,POSearchView.SupplierId,POSearchView.SupplierName," _
                    & " POSearchView.MakerName,POSearchView.ReceivedAllInvoices,POSearchView.Canceled," _
                    & " PoHeaders.StaffName,ConfirmedPoNo.SendingDate, ConfirmedPoNo.ConfirmedDate" _
                    & " FROM POSearchView INNER JOIN Suppliers ON Suppliers.SupplierId=POSearchView.SupplierId " _
                    & " LEFT JOIN ConfirmedPoNo ON PoSearchView.PoNo = ConfirmedPoNo.PONO" _
     ...
     If Not IsNull(dtpFrom.Value) And Not IsNull(dtpTo.Value) Then
          strSQLQuery = strSQLQuery & " PoSearchView.IssuedDate >= " & pfstrQt(Format(dtpFrom.Value, "yyyy/mm/dd")) _
               & " And PoSearchView.IssuedDate <= " & pfstrQt(DateAdd("d", 1, Format(dtpTo.Value, "yyyy/mm/dd")))
     ElseIf Not IsNull(dtpFrom.Value) And IsNull(dtpTo.Value) Then
          strSQLQuery = strSQLQuery & " PoSearchView.IssuedDate >= " & pfstrQt(Format(dtpFrom.Value, "yyyy/mm/dd"))
     ElseIf IsNull(dtpFrom.Value) And Not IsNull(dtpTo.Value) Then
          strSQLQuery = strSQLQuery & " PoSearchView.IssuedDate <= " & pfstrQt(Format(dtpTo.Value, "yyyy/mm/dd"))
     End If
     ...
     strSQLQuery = strSQLQuery & " ORDER BY PoSearchView.PONo, PoSearchView.PoSeq, PoSearchView.CurrencyUnit"
