    ...
    With adoTransView
          Do While Not .EOF
               ...
               Select Case .Fields("TransactionTypeId").Value
                    Case 1
                         strFields = "(SeqNo,TransDate,ItemId,Description,InQty,Division)"
                    Case 3
                         strFields = "(SeqNo,TransDate,ItemId,Description,OutQty,Division)"
                    Case 4
                         strFields = "(SeqNo,TransDate,ItemId,Description,AC,Division)"
               End Select
               ...
               Call clsPrintMenu.WHRecord.SaveToWHRecordWT(strFields, lngSeqNo, pfvarIs_Null(.Fields("TransactedDate").Value) _
                                        , pfvarIs_Null(.Fields("ItemID").Value), pfvarIs_Null(.Fields("Description").Value) _
...
Private Sub subViewWHRecords()
    ...
    With hflxViewWHRecord
          ...
          Do While Not adoRSView.EOF
               ...
               .TextMatrix(lngRow, 3) = adoRSView.Fields("Division").Value
               .TextMatrix(lngRow, 4) = pfvarIs_Null(adoRSView.Fields("InQty").Value, False)
               .TextMatrix(lngRow, 5) = pfvarIs_Null(adoRSView.Fields("OutQty").Value, False)
               .TextMatrix(lngRow, 6) = pfvarIs_Null(adoRSView.Fields("AcQty").Value, False)
               ...
               Else
                    ...
                    dblRunBalance = dblRunBalance + _
                                   ...
                                   pfvarIs_Null(adoRSView.Fields("OutQty").Value, False)
               End If
               .TextMatrix(lngRow, 7) = dblRunBalance
...
Private Sub subGetWHAC()
    ...
    With hflxViewWHRecord
          For intLoop = 1 To .Rows - 1
               ...
               End If
               dblRunBalance = dblRunBalance + _
                                        pfvarIs_Null(.TextMatrix(intLoop, 4), False) - _
                                        pfvarIs_Null(.TextMatrix(intLoop, 5), False)
               .TextMatrix(intLoop, 6) = dblRunBalance
          Next intLoop
    End With
    Set adoRSView = Nothing
End Sub
...
Private Sub subGetWHTotal()
    ...
    With hflxViewWHRecord
          For intLoop = 1 To .Rows - 1
               .TextMatrix(intLoop, 5) = clsPrintMenu.pfvarStockBalance(.TextMatrix(intLoop, 0), cboDivision, dtpTo.Value)
          Next intLoop
    End With
    ...
    Select Case strType
        Case ""
            With hflxViewWHRecord
                ...
                .RowHeight(0) = 450
                For intCol = 0 To 5
                    .TextMatrix(0, intCol) = Choose(intCol + 1, "TRANSACTION DATE", "ITEM ID", "DESCRIPTION", "DIVISION", "IN QTY", "OUT QTY")
                    .ColWidth(intCol) = Choose(intCol + 1, 1400, 1100, 3000, 1500, 1000, 1000)
                Next
            End With
        Case "Total"
lnTotal:
            With hflxViewWHRecord
                ...
                .RowHeight(0) = 450
                For intCol = 0 To 5
                    .TextMatrix(0, intCol) = Choose(intCol + 1, "ITEM ID", "DESCRIPTION", "DIVISION", "IN QTY", "OUT QTY", "ON HAND")
                    .ColWidth(intCol) = Choose(intCol + 1, 1100, 3000, 2000, 1000, 1000, 1000)
                Next
            End With
    End Select
